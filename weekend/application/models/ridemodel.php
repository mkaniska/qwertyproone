...
class RideModel extends CI_Model {
    ...
    function find_matching_rides($data) {
		...
		if (preg_match("/ /i", $data['address'])) {
			...
			if(count($addressString)>1) {
				$locationMatch = "((origin_location LIKE '%$addressString[0]%') OR (destination_location LIKE '%$addressString[0]%'))";
			}else {
		...
		if($data['preferred_gender']!='All') {
			$preferredGender  = " AND preferred_gender = '".$data['preferred_gender']."' ";
		}else {
			$preferredGender = '';
		}		
