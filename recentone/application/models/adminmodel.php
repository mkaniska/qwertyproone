...
class AdminModel extends CI_Model {
    ...
    function get_total_discounts() {
		
		if($this->session->userdata('discount_id')!='' && $this->session->userdata('discount_id')!=0) {
			$this->db->where('pro_discounts.discount_type', $this->session->userdata('discount_id'));
		}
		if($this->session->userdata('discount_city')!='' && $this->session->userdata('discount_city')!='0') {
			$this->db->where('pro_discounts.discount_city', $this->session->userdata('discount_city'));
		}
		$this->db->select();
		$query = $this->db->get('pro_discounts');
		return $query->num_rows();
	}
    ...
    function get_discounts($cp, $pp) {
        
		$this->db->order_by("discount_created_on","DESC");				
		$this->db->select();
		$this->db->limit($cp, $pp);
        $query = $this->db->get('pro_discounts');
		$result_back = array();
	...
	function deleteDiscount($discount_id) {
		...
		$this->db->update('pro_discounts', array('is_deleted'=>'1'));
        if($this->db->affected_rows()>0) {
			return true;
		}else {
			return false;
	...
	function updateDiscount($data,$discount_id) {
		$this->db->where('discount_id', $discount_id);
		$this->db->update('pro_discounts', $data);
        if($this->db->affected_rows()>0) {
			return true;
		}else {
			return false;
	...
	function get_this_discount($discount_id) {
		$this->db->where('pro_discounts.discount_id', $discount_id);
		$this->db->select();		
		$query = $this->db->get('pro_discounts');
		if($query->num_rows() > 0){
			foreach ($query->result() as $row) 
	...
	function insertDiscount($data) {
        $this->db->insert('pro_discounts', $data);
        $inserted_id = $this->db->insert_id();
        return $inserted_id;
	}	
    ...
    function get_discounts_posted($cp, $pp) {
        
		$this->db->order_by("discount_created_on","DESC");
		if($this->session->userdata('discount_type')!='' && $this->session->userdata('discount_type')!='0') {
			$this->db->where('pro_discounts.discount_type', $this->session->userdata('discount_type'));
		}
		if($this->session->userdata('discount_city')!='' && $this->session->userdata('discount_city')!='0') {
			$this->db->where('pro_discounts.discount_city', $this->session->userdata('discount_city'));
		}
