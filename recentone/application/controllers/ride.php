...
class Ride extends CI_Controller {
	...
	public function delete() {
		$ride_id = ($this->uri->segment(3))? $this->uri->segment(3) : 0;
		if(is_numeric($ride_id) && $ride_id>0){
		$isDeleted = $this->RideModel->deleteRide($ride_id);
			if($isDeleted) {
				$this->session->set_flashdata('flash_message', 'Ride Deleted Successfully!');
			}else {
				$this->session->set_flashdata('flash_message', 'No Authorization to Delete this Ride!');
			}
		}else{
	...
	public function matching_rides() {
		...
		$input['preferred_gender'] 	= $this->input->post('preferred_gender');
	...
	public function get_cities() {
		$selected_state = $this->input->post('state_name');
		$selected_city = $this->input->post('select_city');
		$field_name = $this->input->post('field_name');
		$cities_list = $this->CommonModel->cities_of_state($selected_state);
	...
	public function process_instantride() {
		...
		if($this->input->post('post_ride')=='Post') {
			...
			$ride_data['preferred_gender'] 		= $this->input->post('preferred_gender');
			...
			$isValidRideID = $this->RideModel->insertRide($ride_data);
				...
				$tmp_str.= "Preferred Gender : ".$ride_data['preferred_gender']."<br />";
				
				if($ride_data['travel_as']=='driver' || $ride_data['travel_as']=='reference'){
	...
	public function process_ride() {
		...
		if($this->input->post('submitride')=='Submit') {
			...
			$activation_link.= encryptData(array('email'=>$this->input->post('email_address'),'id'=>$isValidUserID));			
			...
			// Send an activation email & notifications
				...
				$email_data['HelloTo'] 			= $user_data['pro_user_full_name'];
