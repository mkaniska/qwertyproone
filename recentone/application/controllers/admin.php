...
class Admin extends CI_Controller {
	...
	public function edit_discount() {
		$this->load->model('CommonModel');
		$data['page_name'] = "admin/edit_discount";
		$discount_id = ($this->uri->segment(3))? $this->uri->segment(3) : 0;
		$rows = $this->AdminModel->get_this_discount($discount_id);
		$data['cities_list'] = $this->CommonModel->cities_list('Agra');
		$data['discount'] = $rows[0];
		$data['menu'] = "edit_discount";
		$data['title'] = SITE_ADMIN_TITLE." :: Edit Discount";
		$this->load->view('layouts/admin_layout', $data);
	}	
	...
	public function delete_discount() {
		$discount_id = ($this->uri->segment(3))? $this->uri->segment(3) : 0;
		$isDeleted = $this->AdminModel->deleteDiscount($discount_id);
		if($isDeleted) {
			$this->session->set_flashdata('flash_message', 'Offer Deleted Successfully!');
		}else {
			$this->session->set_flashdata('flash_message', 'Error on Deleting Offer!');
	...
	public function process_updatediscount() {
	
		if($this->session->userdata('admin_user_id')==''){redirect('admin/login');}
		$discount_id = $this->input->post('discount_id');
		if($this->input->post('editDiscount')=='Update') {
			
			$inp_data['discount_city'] 			= $this->input->post('discount_city');
			$inp_data['discount_title'] 		= $this->input->post('discount_title');
			$inp_data['discount_type'] 			= $this->input->post('discount_type');
			$inp_data['discount_notes']			= $this->input->post('discount_notes');
			
			$inp_data['discount_provider'] 		= $this->input->post('discount_provider');
			$inp_data['contact_person'] 		= $this->input->post('contact_person');
			$inp_data['contact_phone'] 			= $this->input->post('contact_phone');
			
			// File uploading here
			if($_FILES['userfile']['name']!='') {			
				$fconfig['upload_path'] = 'discount_pictures/';
				$fconfig['allowed_types'] = 'gif|jpg|png|GIF|JPG|JPEG|PNG';
				$fconfig['max_size']	= '4000';
				$fconfig['max_width']  = '2024';
				$fconfig['max_height']  = '2024';
				...
				if(!$this->upload->do_upload()) {
					$this->session->set_flashdata('flash_message', 'Invalid Picture Uploaded!');
					$this->session->set_flashdata('flash_data', $inp_data);
					redirect('admin/edit_discount/'.$discount_id);
				} else {
					$inp_data['discount_picture'] = $fconfig['file_name'];
					$upload_data =$this->upload->data();
					$rconfig['image_library'] = 'gd2';
					$rconfig['source_image'] = $upload_data['full_path'];
					$rconfig['create_thumb'] = TRUE;
			...
			$temp_details = $this->AdminModel->get_this_discount($discount_id);
			
			$isUpdated = $this->AdminModel->updateDiscount($inp_data, $discount_id);
			
			if($isUpdated) {
				if($_FILES['userfile']['name']!='') {
					unlink('discount_pictures/'.$temp_details[0]->discount_picture);
					unlink('discount_pictures/'.str_replace(".","_thumb.",$temp_details[0]->discount_picture));
					unlink('discount_pictures/'.str_replace(".","_normal_thumb.",$temp_details[0]->discount_picture));
				}
				$this->session->set_flashdata('flash_message', 'Discount Updated Successfully!');
				redirect('admin/discount_list');
			}else {
				$this->session->set_flashdata('flash_message', 'Invalid Details Entered!');
				redirect('admin/edit_discount/'.$discount_id);
			}
	...
	public function process_updateads() {
		...
		if($this->input->post('updateAds')=='Update') {
			...
			if($isUpdated) {
				if($_FILES['userfile']['name']!='') {
					unlink('ads_images/'.$temp_details[0]->ads_image);
					unlink('ads_images/'.str_replace(".","_thumb.",$temp_details[0]->ads_image));
				}
	...
	public function adddiscount() {
		$data['page_name'] = "admin/adddiscount";
		$data['cities_list'] = $this->CommonModel->cities_list('Agra');
		$data['menu'] = "adddiscount";
		$data['title'] = SITE_ADMIN_TITLE." :: Add Discount";
		$this->load->view('layouts/admin_layout', $data);
	}

	public function process_adddiscount() {
	
		if($this->session->userdata('admin_user_id')==''){redirect('admin/login');}
		
		if($this->input->post('addDiscount')=='Submit') {
			
			$inp_data['discount_city'] 			= $this->input->post('discount_city');
			$inp_data['discount_title'] 		= $this->input->post('discount_title');
			$inp_data['discount_type'] 			= $this->input->post('discount_type');
			$inp_data['discount_notes']			= $this->input->post('discount_notes');
			$inp_data['discount_status'] 		= 1; 
			$inp_data['discount_created_on'] 	= time();
			...
			$isValidDiscountID = $this->AdminModel->insertDiscount($inp_data);
			
			if($isValidDiscountID>0) {
	...
	public function discount_list() {
		...
		$config['num_links']		= 3;
		$config['per_page'] 		= 5;
		$config['base_url'] 		= base_url().'admin/discount_list/';
		$config['use_page_numbers'] = TRUE;
        $config['cur_tag_open'] 	= "<li><span><b>";
        $config['cur_tag_close'] 	= "</b></span></li>";
		$config['full_tag_open'] 	= '<ul>';
		...
		$data['total']     = $config['total_rows'];
		$data['page_name'] = "admin/discount_list";
		$data['menu'] = "discount_list";
		if($page!=0) {$start = ($page*5)-5;}else{$start = 0;}
		$data['discount_list'] = $this->AdminModel->get_discounts($config["per_page"], $start);
		$data['title'] = SITE_ADMIN_TITLE." :: List of Discounts";
		$this->pagination->initialize($config); 
		$data['pagelink'] = $this->pagination->create_links();
	...
	public function edit_hruser() {
		$hr_id = ($this->uri->segment(3))? $this->uri->segment(3) : 0;
		$info = $data['user_details'] = $this->UserModel->get_user_details($hr_id);
		//print_r($info);exit;
		$data['cities_list'] = $this->CommonModel->cities_list($info->pro_user_state);
		$data['state_list'] = $this->CommonModel->states_list();
		$data['page_name'] = "admin/edit_hruser";
		$data['userid'] = $hr_id;		
		$data['menu'] = "edit_hruser";		
		$data['title'] = SITE_ADMIN_TITLE." :: Edit HR User";
		$this->load->view('layouts/admin_layout', $data);
	}

	public function process_updatehruser() {
		
		if($this->input->post('updateHRUser')=='Update') {
			$posted_data['pro_user_type'] 		= 3; // HR
			...
			$isValidID = $this->UserModel->updateUserDetails($posted_data,$user_id);
			
			if($isValidID) {
				$this->session->set_flashdata('flash_message', 'Successfully Updated the HR Account !');
				redirect('admin/company_details/'.$posted_data['pro_corporate_id']);
			} else {
				$this->session->set_flashdata('data_back', $posted_data);
				$this->session->set_flashdata('flash_message', 'Please enter all the details');
				redirect('admin/edit_hruser/'.$user_id);
			}
		}else{
			$this->session->set_flashdata('flash_message', 'Invalid Request!');
			redirect('admin/edit_hruser/'.$user_id);
